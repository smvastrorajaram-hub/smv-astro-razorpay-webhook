...
// ---------- Translation-only API (does not modify any existing business/payment data) ----------
// Translation is performed only when the frontend requests it. Original questions,
// answers, messages and other Firestore content are never overwritten by this route.
const translationCache = new Map();
const translationCacheMax = 2000;
function rememberTranslation(key, value) {
  if (translationCache.size >= translationCacheMax) {
    const first = translationCache.keys().next().value;
    if (first) translationCache.delete(first);
  }
  translationCache.set(key, value);
}
async function translateOneText(text, target) {
  const source = String(text ?? "").trim();
  const lang = String(target || "en").toLowerCase();
  if (!source || lang === "en") return String(text ?? "");
  if (!/^(ta|hi)$/.test(lang)) throw new Error("Unsupported translation language.");
  if (/^[\d\s₹$€£.,:/()_+\-#%]+$/.test(source)) return String(text ?? "");
  const key = `${lang}|${source}`;
  if (translationCache.has(key)) return translationCache.get(key);

  // Google public translation endpoint is used so no new secret is required in Render.
  // If GOOGLE_TRANSLATE_API_KEY is later configured, this can be upgraded without
  ...
  try {
    ...
    if (!r.ok) throw new Error(`Translation provider returned HTTP ${r.status}`);
    const data = await r.json();
    const translated = Array.isArray(data?.[0])
      ? data[0].map(x => Array.isArray(x) ? String(x[0] || "") : "").join("")
      : "";
    if (!translated) throw new Error("Translation provider returned no translation.");
    const value = translated;
    rememberTranslation(key, value);
    return value;
  } finally { clearTimeout(timer); }
}
app.post("/translate-batch", express.json({ limit: "200kb" }), async (req, res) => {
  try {
    const target = String(req.body?.target || "").toLowerCase();
    const texts = Array.isArray(req.body?.texts) ? req.body.texts.map(x => String(x ?? "")) : [];
    if (!/^(ta|hi|en)$/.test(target)) return res.status(400).json({ error: "Translation language must be en, ta or hi." });
    if (texts.length > 60) return res.status(400).json({ error: "Too many translation items in one request." });
    const unique = [...new Set(texts.map(x => x.trim()).filter(Boolean))];
    const results = {};
    // Small concurrency keeps Render and the translation provider stable.
    for (let i = 0; i < unique.length; i += 6) {
      const part = unique.slice(i, i + 6);
      const translated = await Promise.all(part.map(async t => {
        try { return [t, await translateOneText(t, target)]; }
        catch (e) { console.warn("Translation failed:", e?.message || e); return [t, t]; }
      }));
      translated.forEach(([from, to]) => { results[from] = to; });
    }
    return res.json({ success: true, target, translations: results });
  } catch (e) {
    console.error("Translation endpoint error:", e);
    return res.status(500).json({ error: e?.message || "Translation service failed." });
  }
});
