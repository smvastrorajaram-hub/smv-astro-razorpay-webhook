...
async function nextPublicId(prefix, period) {
  const ref = db.collection("smv_counters").doc(`${prefix === "CS" ? "customer" : "astrologer"}_${period}`);
  return db.runTransaction(async tx => {
    const snap = await tx.get(ref);
    const next = (snap.exists ? Number(snap.data()?.lastNumber || 0) : 0) + 1;
    tx.set(ref, { lastNumber: next, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
...
app.post("/register-customer-profile", async (req, res) => {
  ...
  try {
    ...
    const now = new Date();
    const period = String(now.getMonth() + 1).padStart(2, "0") + String(now.getFullYear()).slice(-2);
    const publicId = await nextPublicId("CS", period);
    await ref.set({
      uid: user.uid, name, phone, email: user.email || "", role: "customer",
      status: "active", publicId, emailVerificationRequired: true,
...
app.post("/register-astrologer-profile", async (req, res) => {
  ...
  try {
    ...
    const now=new Date(), period=String(now.getMonth()+1).padStart(2,"0")+String(now.getFullYear()).slice(-2), publicId=await nextPublicId("AT",period);
